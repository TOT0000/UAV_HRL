    ...
    def test_train_resume_and_evaluation_emit_no_task_role_diagnostics(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(main([
                "td3_ratio", "--episodes", "1", "--episode-seconds", "1",
                "--checkpoint-interval", "1", "--roi-count", "2",
            ...
            self.assertEqual(main([
                "evaluate", str(run_dir), "--checkpoint-episode", "2", "--smoke",
            ]), 0)
            evaluation_dir = next((run_dir / "evaluation" / "ep_2").iterdir())
            evaluation_metadata = json.loads(
                (evaluation_dir / "run_metadata.json").read_text(encoding="utf-8")
            )
            self.assertFalse((evaluation_dir / "relay_diagnostics.json").exists())
            self.assertNotIn("relay_diagnostics_filename", evaluation_metadata)
