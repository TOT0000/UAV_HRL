import numpy as np
from Simulator_Rand import Simulator
...
def train():
    ...
    # ===== Checkpoint / Resume =====
    ckpt_root = "checkpoints_rand_td3_dinkel"
    ...
    for episode in range(start_episode, total_episodes):
        ...
        avg_fov_episode = fov_accum / fov_count
    ...
    plt.title("Rand select Training - Total Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.title("Rand TD3 with Dinkelback Learning Trend of FOV Coverage")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.title("Rand TD3 with Dinkelbach Violation Rate over Episodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
