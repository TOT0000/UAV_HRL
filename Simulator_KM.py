...
class Simulator:
    ...
    #     return 80 * np.exp(-((x - 600)**2 + (y - 600)**2) / 1e5)  # 中心在 (500,600)
    ...
    def get_available_uav_ids(self):
        ...
        for uav in self.UAVs:
            # if (
            #     uav.task_type == "Search" and
            #     not uav.is_low_battery(threshold=30) and
            #     uav.get_buffer_bits() < 0.8 * uav.max_buffer_bits  # optional: 避免 buffer 太滿
            # ):
    ...
    def assign_tasks(self):
        ...
        assigner.assign_tasks(uav_id_list, self.task_list, K=1, mode="KM")
        assigner.build_uav_tasks_from_assignment()# 分配結果改成任務列表
     # ===================初始化地圖網格======================
    # def initialize_visited_map(self):
    #     self.grid_w = int(self.env_width / self.grid_size)
    #     self.grid_h = int(self.env_height / self.grid_size)
    #     self.visited_bitmap = np.zeros((self.grid_w, self.grid_h), dtype=bool)
    # ====================更新探索區域=====================
    ...
    def calculate_fov_reward(self, uav_id, lamda_EE, E_mob=None, debug=False):
        uav = self.uav_dict[uav_id]
        ...
        reward = r_centered + r_gain - r_overpen - lamda_EE * r_energy
    ...
    def calculate_fov_reward(self, uav_id, lamda_EE, E_mob=None, debug=False):
        ...
        reward = r_centered + r_gain - r_overpen - lamda_EE * energy_term
        reward = float(np.clip(reward, -2.0, 4.0))
    ...
    def calculate_fov_reward_wo_Dinkel(self, uav_id, E_mob=None, debug=False):
        ...
        if r_energy <= 0:
            ...
            reward = (r_centered + r_gain - r_overpen) - 0.2 * r_energy
        ...
        return reward, err, fov
    def calculate_search_reward(self, uav_id, t, lamda_EE, E_mob=None):
        uav = self.uav_dict[uav_id]
        if E_mob is None:
            E_mob = float(getattr(uav, "move_energy_step", 0.0))
        ...
        ratio = (newly_explored / max(1, fov_cells))

        # print(
        #     f"[SEARCH] raw={raw:.3f} explore={explore:.3f} "
        #     f"new_area={newly_explored:.0f} fov_cells={fov_cells} ratio={ratio:.4f} "
        #     f"no_move_penalty={no_move_penalty:.3f} overlap_penalty={overlap_penalty:.3f} "
        #     f"energy_pen={(lamda_EE * energy_term):.3f} lam={lamda_EE:.3f}"
        # )
    ...
    def calculate_search_reward_wo_Dinkel(self, uav_id, t, E_mob=None):
        ...
        if energy_term  <= 0:
            ...
            raw = (explore - overlap_penalty - no_move_penalty) - 0.2 * energy_term 
    ...
    def rand_boundary_point(self, w, h):
        side = random.randint(0, 3)  # 0:left, 1:right, 2:bottom, 3:top
        if side == 0:   # left
            return 0, random.uniform(0, h)
        if side == 1:   # right
            return w, random.uniform(0, h)
        if side == 2:   # bottom
            return random.uniform(0, w), 0
        else:           # top
    ...
    def reset_environment(self):
        ...
        for i in range(self.num_GT):
            x, y = self.rand_boundary_point(self.env_width, self.env_height)
            sr = SRTeam(id=i)
