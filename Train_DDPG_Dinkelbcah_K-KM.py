...
def train():
    ...
    # ===== Checkpoint / Resume =====
    ckpt_root = "checkpoints_K-KM_ddpg_dinkel"
    ...
    "xs": x_episode,
    "Val": total_reward
    }).to_csv(os.path.join(csv_dir,"K-KM_DDPG_results_total.csv"), index=False)
    pd.DataFrame({
    "xs": smoothed_x,
    "Val": smoothed_total
    }).to_csv(os.path.join(csv_dir,"K-KM_DDPG_AVG_results_total.csv"), index=False)
    ...
    plt.title("K-KM DDPG with Dinkelback Learning Trend of FOV Coverage")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.title("K-KM DDPG with Dinkelbach Violation Rate over Episodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
