...
def train():
    ...
    next_hop_by_uav={}
    # ===== Checkpoint / Resume =====
    ckpt_root = "checkpoints_K-KM_td3"
    ...
    for episode in range(total_episodes):
        ...
        if episode >= warmup_episodes:
            if movement_buffer_search.size > current_batch_size:
                Model_TD3_search.update(movement_buffer_search, current_batch_size)
            if movement_buffer_fov.size > current_batch_size:
                Model_TD3_fov.update(movement_buffer_fov, current_batch_size)
    ...
    plt.title("TD3 W/O Dinkelbach Training - Total Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.title("TD3_WO_Dinkelbach Learning Trend of FOV Coverage")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.title("TD3_WO_Dinkelbach Violation Rate over Episodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
