...
def train():
    ...
    start_episode = 0
    # ===== Checkpoint / Resume =====
    ckpt_root = "checkpoints_our_method"
    ...
    for episode in range(total_episodes):
        ...
        for t in range(T_slot):
            ...
            if t% 4 == 0:
                for uav_id in active_uav_ids:
                    ...
                    for task in task_list:
                        ...
                        state = rout.get_state_ta(env, uav_id, backlog_bits=backlog_bits)
                        if task_type == "Search":
                            ...
                            dx, dy, dz =0, 0, 0
                            E_hover = uav.apply_movement(dx, dy, dz, energy_model=env.energy_model, step_time=1.0, mobility_params=env.mobility_params)
            ...
            if t % 4 == 0:   # 每 1 秒更新一次 (0.25 × 4)
                ...
                gs_ok = (env.gs_capacity > 0.1) if (env.gs_capacity is not None) else np.zeros(env.num_UAV, dtype=bool)
            ...
            ns_cache = {
                uid: rout.get_state_ta(env, uid, backlog_bits=backlog_bits_after)
                for uid in uavs_with_pkts
            }
            ...
            for uid in uavs_with_pkts:
                ...
                action = int(next_hop_by_uav.get(uid, uid))   #  統一 action
        ...
        if (episode >= WARMUP_LAMBDA) and (episode % K_UPDATE == 0) and (total_energy > 1e-6):
            ...
            ratio_raw = (total_bn_rate_bps / 1e9) / total_energy     
            ratio = SCALE_LAMBDA * ratio_raw                           
    ...
    # os.makedirs("Our-mathed", exist_ok=True)
    # Model_TD3_search.save("Our-mathed/uav_search")
    # Model_TD3_fov.save("Our-mathed/uav_fov")
    # Model_DDQN.save("Our-mathed/uav_ddqn")
    ...
    plt.title("Our-mathed Dinkelbach Training - Total Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    pd.DataFrame({
        "xs": x_episode,
        "Val": total_reward
    }).to_csv(os.path.join(csv_dir, "Our_results_total.csv"), index=False)

    pd.DataFrame({
        "xs": smoothed_x,
        "Val": smoothed_total
    }).to_csv(os.path.join(csv_dir, "Our_AVG_results_total.csv"), index=False)
    #===================== 畫Avg fov coverage=============================
    x_episode = list(range(total_episodes))
    start_idx = 2
    ...
    plt.title("Our-mathed Learning Trend of FOV Coverage")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.title("Our-method Violation Rate over Episodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
