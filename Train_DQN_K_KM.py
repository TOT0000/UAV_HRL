...
def train():
    ...
    Model_DDQN = DQN(state_dim, routing_dim)
    ...
    for episode in range(start_episode, total_episodes):
        ...
        # print("[DBG] after reset: visited_mean=", float(env.visited_bitmap.mean()), "sum=", int(env.visited_bitmap.sum()))
        routing_mask_cache = {}
        ...
        if ((episode + 1) % CKPT_EVERY) == 0:
            ...
            Model_DDQN.save(os.path.join(save_dir, "uav_dqn"))
    ...
    plt.title("DQN with Dinkelback Learning Trend of FOV Coverage")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.title("DQN with Dinkelbach Violation Rate over Episodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
