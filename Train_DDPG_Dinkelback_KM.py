import numpy as np
from Simulator_KM import Simulator
...
def train():
    ...
    # ===== Checkpoint / Resume =====
    ckpt_root = "checkpoints_KM_ddpg_dinkel"
    ...
    for episode in range(start_episode, total_episodes):
        ...
        # use_dinkel = (episode >= warmup_episodes)
        # if use_dinkel:
        #     max_dinkel_iters = 3
        # else:
        #     max_dinkel_iters = 1
        # for d_iter in range(max_dinkel_iters):
        # Reset env, simulate with current lambda_ee
                ...
                for uav_id in active_uav_ids:
                    ...
                    for task in task_list:
                        ...
                        if task_type == "Search":
                            ...
                            total_energy += E_mob
                            next_state = rout.get_state(env, uav_id, backlog_bits=backlog_bits)
                            movement_buffer_fov.add(state, movement_action, next_state, fov_reward, done=False, tag_gt=env.num_GT)
        ...
        if episode >= warmup_episodes:
            if movement_buffer_search.size > current_batch_size:
                Model_DDPG_search.train(movement_buffer_search, env.num_GT, current_batch_size)
            if movement_buffer_fov.size > current_batch_size:
                Model_DDPG_fov.train(movement_buffer_fov, env.num_GT, current_batch_size)
    ...
    plt.title("K-KM DDPG Dinkelbach Training - Total Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xlabel("Episode")
    plt.ylabel("KM-DDPG_Routing Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xlabel("Episode")
    plt.ylabel("KM-DDPG_Search Reward")
    ...
    "xs": x_episode,
    "Val": total_reward
    }).to_csv(os.path.join(csv_dir,"KM_DDPG_results_total.csv"), index=False)
    pd.DataFrame({
    "xs": smoothed_x,
    "Val": smoothed_total
    }).to_csv(os.path.join(csv_dir,"KM_DDPG_AVG_results_total.csv"), index=False)
    ...
    plt.title("KM-DDPG with Dinkelback Learning Trend of FOV Coverage")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.title("KM-DDPG with Dinkelbach Violation Rate over Episodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
