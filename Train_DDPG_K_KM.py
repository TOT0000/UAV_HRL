...
def train():
    ...
    moving_dim = 3
    max_action = 1
    Model_DDPG_search = DDPG(state_dim, moving_dim, max_action )
    Model_DDPG_fov = DDPG(state_dim, moving_dim, max_action )
    ...
    for episode in range(start_episode, total_episodes):
        ...
        for t in range(T_slot):
            ...
            if t% 4 == 0:
                for uav_id in active_uav_ids:
                    ...
                    for task in task_list:
                        ...
                        #  取得狀態並讓 actor 選擇動作
                        state = state_cache[uav_id]
                        if task_type == "Search":
                            raw_action = Model_DDPG_search.select_action(state, episode=episode)
                            movement_action = Model_DDPG_search.decode_action(raw_action)
                            ...
                            raw_action = Model_DDPG_fov.select_action(state, episode=episode)
                            movement_action = Model_DDPG_fov.decode_action(raw_action)
                            dx, dy, dz = movement_action
                            E_mob = uav.apply_movement(dx, dy, dz, energy_model=env.energy_model, step_time=1.0)
                            fov_reward, _, current_fov = env.calculate_fov_reward(uav_id, lambda_EE, E_mob)
                            normalized_fov_reward = fov_reward 
                            # print(current_fov)
                            scaled_fov_reward = normalized_fov_reward # 你可在這裡加權
                            total_energy += E_mob
                            next_state = rout.get_state(env, uav_id, backlog_bits=backlog_bits)
                            movement_buffer_fov.add(state, raw_action, next_state, scaled_fov_reward, done=False, tag_gt=env.num_GT)
                            episode_fov_reward += scaled_fov_reward
                            episode_total_reward+= scaled_fov_reward
            ...
            env.update_source_uavs()
                # 🐞 Debug log 可加
                # print(f"[MultiTask] UAV {uav_id} → Task {current_task_index} ({task_type}), Pos = {uav.target_position}, Reward = {move_reward:.2f}")
            # ===========設定source uav=============================
            # start = time.perf_counter()
        ...
        if (episode % K_UPDATE == 0) and (total_energy > 1e-6):
        ...
        if episode >= warmup_episodes:
            if movement_buffer_search.size > current_batch_size:
                Model_DDPG_search.train(movement_buffer_search, current_batch_size)
            if movement_buffer_fov.size > current_batch_size:
                Model_DDPG_fov.train(movement_buffer_fov, current_batch_size)
        ...
        print(f"Average FOV: {avg_fov_episode:.3f}")
        #  ===== Save checkpoint =====
        if ((episode + 1) % CKPT_EVERY) == 0:
            ...
            # model files
            Model_DDPG_search.save(os.path.join(save_dir, "uav_search"))
            Model_DDPG_fov.save(os.path.join(save_dir, "uav_fov"))
    ...
    plt.title("DDPG W/O Dinkelbach Training - Total Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    # plt.savefig("Total_reward.png")  # 可加上存圖
    plt.show()
    ...
    plt.title("DDPG_W_Dinkelbach Learning Trend of FOV Coverage")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.title("DDPG_W_Dinkelbach Violation Rate over Episodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
